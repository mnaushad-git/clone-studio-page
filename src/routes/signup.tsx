...
export const Route = createFileRoute("/signup")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Sign up for a Terrific Bites account to order desserts, save favorites, and earn rewards." },
      { property: "og:title", content: "Create An Account — Terrific Bites" },
      { property: "og:description", content: "Join Terrific Bites for fresh desserts, custom cakes and member rewards." },
...
function Field({ label, ...props }: { label: string } & React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <div>
      ...
      <input
        {...props}
        className="w-full border border-border rounded-md px-4 py-3 text-sm focus:outline-none focus:border-primary bg-white"
      />
    </div>
  );
}
    ...
    <div className="min-h-screen bg-secondary flex items-stretch">
      ...
      <div className="hidden md:flex md:w-1/2 items-center justify-center p-12 relative overflow-hidden">
        ...
        <div className="text-center max-w-md relative">
          ...
          <img src={illustration} alt="Terrific Bites app illustration" width={520} height={520} className="mt-10 w-full max-w-md mx-auto" />
        </div>
      </div>
      ...
      <div className="w-full md:w-1/2 bg-background rounded-l-3xl p-8 md:p-14 flex items-center">
        <div className="w-full max-w-md mx-auto">
          ...
          <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
            ...
            <Field label="Email" type="email" placeholder="mvpuxdesign@gmail.com" value={form.email} onChange={set("email")} />
          ...
          <div className="space-y-3">
            <button className="w-full bg-secondary rounded-md py-3 text-sm font-medium flex items-center justify-center gap-3 hover:bg-secondary/70 transition">
          ...
          <p className="text-center text-sm text-muted-foreground mt-6">
            Already have an account? <Link to="/signup" className="font-semibold text-foreground hover:text-primary">Sign In</Link>
          </p>
        </div>
      </div>
    </div>
