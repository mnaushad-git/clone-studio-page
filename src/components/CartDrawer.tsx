      ...
      <aside className="hidden md:flex w-72 bg-[#f7e6e6] h-full flex-col shadow-2xl">
        <div className="px-5 py-5 border-b border-black/5">
          <h3 className="font-display text-lg text-primary leading-tight">
            {t("cartCompleteYourOrderLine1")}<br />{t("cartCompleteYourOrderLine2")}
          </h3>
        </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
            ...
            <div className="border border-border rounded-lg overflow-hidden">
              ...
              {isAuthed && (
                <div className="px-4 py-3 border-t border-border text-sm space-y-1.5">
                  <div className="flex justify-between">
                    ...
                    <span>{delivery === 0 ? t("cartFreeLabel") : `SAR ${delivery.toFixed(2)}`}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground flex items-center gap-1.5">
                      <MapPin className="h-4 w-4" /> {t("cartDeliverToLabel")} <span className="text-foreground font-medium">{t("cartYourAddress")}</span>
                    </span>
                  </div>
                </div>
              )}
              <div className="px-4 py-3 border-t border-border text-sm space-y-1.5">
                ...
                {isAuthed && discount > 0 && (
                  <div className="flex justify-between text-primary">
                    <span className="flex items-center gap-1.5">
                      <Tag className="h-4 w-4" /> {t("cartDiscountLabel")} ({activePromo?.code} −{activePromo?.percent}%)
                    </span>
                ...
                {isAuthed && pointsDisc > 0 && (
                  <div className="flex justify-between text-primary">
                    <span className="flex items-center gap-1.5"><Wallet className="h-4 w-4" /> {t("cartPointsLabel")} ({redeemed} {t("cartPtsAbbrev")})</span>
                ...
                {activePromo ? (
                  <div className="flex items-center justify-between text-sm">
                    <span>
                      ...
                      <span className="text-muted-foreground">{t("cartAppliedLabel")} (−{activePromo.percent}%)</span>
                    </span>
                    <button
            ...
            {isAuthed && points > 0 && (
              <div className="border border-border rounded-lg p-4 space-y-2">
                ...
                <p className="text-xs text-muted-foreground">{t("cartBalanceLabel")} {points} {t("cartPtsAbbrev")} ({loyaltyCfg.redeemRate} {t("cartPtsAbbrev")} = SAR 1)</p>
                {redeemed > 0 ? (
                  <div className="flex items-center justify-between text-sm">
                    <span>{t("cartUsingLabel")} <span className="font-semibold">{redeemed}</span> {t("cartPtsAbbrev")} (−SAR {pointsDisc.toFixed(2)})</span>
                    <button onClick={() => loyalty.clearRedeem()} className="text-xs text-primary underline hover:no-underline">{t("cartRemove")}</button>
                  </div>
                ) : (
                  <div className="flex gap-2">
                    <input
            ...
            {!isAuthed && (
              <div className="border border-dashed border-primary/40 bg-primary/5 rounded-lg p-4 text-sm text-foreground/80">
                <p>
                  <Link to="/login" search={{ redirect: "/customize" } as never} onClick={onClose} className="font-semibold text-primary underline underline-offset-2">{t("cartSignInLinkLabel")}</Link>{" "}
            ...
            <div className="space-y-2">
              {belowMin && (
                <p className="text-xs text-destructive text-center">
                  {t("cartMinOrderPrefix")} SAR {minOrder.toFixed(2)}. {t("cartMinOrderAddPrefix")} SAR {(minOrder - subtotal).toFixed(2)} {t("cartMinOrderAddSuffix")}
                </p>
              )}
              {isAuthed ? (
                belowMin ? (
            ...
            {/* Your Items */}
            <div>
              <h4 className="font-semibold text-primary mb-3">{t("cartYourItemsLabel")} ({count})</h4>
                  ...
                  return (
                    <div key={it.lineId} className="flex gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-muted-foreground mt-1">{t("cartDeliveryLocal")}</p>
