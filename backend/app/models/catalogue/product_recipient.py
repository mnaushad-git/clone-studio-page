"""Product-to-recipient mapping. Composite primary key (product_id, recipient_id) — no
...
class CatalogueProductRecipient(TimestampMixin, Base):
    __tablename__ = "catalogue_product_recipients"
    __table_args__ = (
        CheckConstraint(
    ...
    recipient_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("catalogue_recipients.id"), primary_key=True
    )
