"""Manually drive the paid-order -> email/SMS confirmation consumer (normally run by
Celery Beat every 30s, see app/workers/celery_app.py). Useful for local verification
without a Redis broker running, or as an ops fallback if Beat/the worker is down.
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"Order confirmations: processed={summary.processed}")
